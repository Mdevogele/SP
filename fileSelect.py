...
SQL_Arc = '''SELECT file FROM obslog WHERE
    ...
    AND Disperser LIKE :Disperser AND CentWave=:CentWave
    '''
...
SQL_SciSpec = '''SELECT file FROM obslog WHERE
    use_me=1 AND ObsType='OBJECT' AND Object LIKE :Object
    AND Instrument=:Instrument AND RoI=:RoI AND CcdBin=:CcdBin
    AND Disperser LIKE :Disperser AND CentWave=:CentWave
    '''


SQL_SciSpecSimp = '''SELECT file FROM obslog WHERE
...
SQL_Ast = '''SELECT file FROM obslog WHERE
    use_me=1 AND ObsType='OBJECT' AND ObsClass='science' AND Object LIKE :Object
    '''    

SQL_Object = '''SELECT object FROM obslog WHERE
    use_me=1 AND ObsType='OBJECT'
    '''  

SQL_CentWave = '''SELECT centwave FROM obslog WHERE
    use_me=1 AND ObsType='OBJECT' AND ObsClass='science'
    '''  

SQL_FileCentWave = '''SELECT file FROM obslog WHERE
    use_me=1 AND ObsType='OBJECT' AND ObsClass='science' AND Object LIKE :Object
    AND CentWave=:CentWave 
    '''      

SQL_DateObs = '''SELECT DateObs FROM obslog WHERE
    use_me=1 AND ObsType='OBJECT' AND ObsClass='science'
    '''  

SQL_CcdBin = '''SELECT CcdBin FROM obslog WHERE
    use_me=1 AND ObsType='OBJECT' AND ObsClass='science'
    '''  

SQL_Disp = '''SELECT Disperser FROM obslog WHERE
    use_me=1 AND ObsType='OBJECT' AND ObsClass='science'
    '''      

SQL_TimeFlat = '''SELECT TimeObs FROM obslog WHERE
    use_me=1 AND ObsType='FLAT' AND ObsClass LIKE '%Cal'
    AND Instrument=:Instrument AND RoI=:RoI AND CcdBin=:CcdBin
    AND Disperser LIKE :Disperser AND CentWave=:CentWave
    '''
SQL_TimeSpec = '''SELECT TimeObs FROM obslog WHERE
    use_me=1 AND ObsType='OBJECT' AND Object LIKE :Object
    AND Instrument=:Instrument AND RoI=:RoI AND CcdBin=:CcdBin
    AND Disperser LIKE :Disperser AND CentWave=:CentWave
    '''

SQL_TimeArc = '''SELECT TimeObs FROM obslog WHERE
    use_me=1 AND ObsType='ARC' AND ObsClass LIKE '%Cal'
    AND Instrument=:Instrument AND RoI=:RoI AND CcdBin=:CcdBin
    AND Disperser LIKE :Disperser AND CentWave=:CentWave
    '''
...
SQL_TYPES = {
             ...
             'ast':SQL_Ast,
             'object':SQL_Object,
             'centwave':SQL_CentWave,
             'filecentwave':SQL_FileCentWave,
             'dateobs':SQL_DateObs,
             'ccdbin':SQL_CcdBin,
             'disperser':SQL_Disp,
             'timeflat':SQL_TimeFlat,
             'timespec':SQL_TimeSpec,
             'timearc':SQL_TimeArc
             }

def createQuery(listType, queryDict, Date = True):
...
def objectListQuery(dbFile, query, selectDict):
    '''From a DB of observation metadata, return a list of files matching the
        specified criteria.
    ...
    with sqlite3.connect(dbFile) as db:
        ...
        c = db.cursor()
        c.execute(query, selectDict)
        fileList = [row['Object'] for row in c.fetchall()]
    return fileList


def centwaveListQuery(dbFile, query, selectDict):
    '''From a DB of observation metadata, return a list of files matching the
        specified criteria.
    ...
    with sqlite3.connect(dbFile) as db:
        ...
        c = db.cursor()
        c.execute(query, selectDict)
        fileList = [row['CentWave'] for row in c.fetchall()]
    return fileList

def dateListQuery(dbFile, query, selectDict):
    '''From a DB of observation metadata, return a list of files matching the
        specified criteria.
    ...
    with sqlite3.connect(dbFile) as db:
        ...
        c = db.cursor()
        c.execute(query, selectDict)
        fileList = [row['DateObs'] for row in c.fetchall()]
    return fileList

def timeListQuery(dbFile, query, selectDict):
    '''From a DB of observation metadata, return a list of files matching the
        specified criteria.
    ...
    with sqlite3.connect(dbFile) as db:
        ...
        c = db.cursor()
        c.execute(query, selectDict)
        fileList = [row['TimeObs'] for row in c.fetchall()]
    return fileList

def ccdbinListQuery(dbFile, query, selectDict):
    '''From a DB of observation metadata, return a list of files matching the
        specified criteria.
    ...
    with sqlite3.connect(dbFile) as db:
        ...
        c = db.cursor()
        c.execute(query, selectDict)
        fileList = [row['CcdBin'] for row in c.fetchall()]
    return fileList

def disperserListQuery(dbFile, query, selectDict):
    '''From a DB of observation metadata, return a list of files matching the
        specified criteria.
    ...
    with sqlite3.connect(dbFile) as db:
        ...
        c = db.cursor()
        c.execute(query, selectDict)
        fileList = [row['Disperser'] for row in c.fetchall()]
    return fileList
